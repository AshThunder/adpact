...
def test_user_balance_no_withdraw(direct_vm, direct_deploy, direct_alice):
    # Deploy the contract
    ...
    print("\n[Simulating Balance Increase Test - No Withdrawal]")
    ...
    formatted_gen = f"{int(bal_after_retention) / 1e18:.2f}"
    print(f"-> Human Readable Escrow Payout released: {formatted_gen} GEN")
    print(f"-> [STAYING ACTIVE] The balance of {user_hex} remains credited at {formatted_gen} GEN.")
    print("[SUCCESS] All virtual escrow balance steps simulated successfully and left active for manual withdrawal testing!")
