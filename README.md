...
**AdPact solves this through on-chain AI, account verification, and multi-stage escrows:**
1. **𝕏 (Twitter) Account Linking & Proof**: Creators connect their wallet and link their X account, enabling verifiable reputation tracking and cryptographic proof of social ownership.
2. **Decentralized AI Reputation Scoring**: GenLayer validator nodes evaluate creator accounts on-chain, computing an authentic **Trust Score (0–100)** to filter out bot networks and fake followers without relying on expensive Web2 APIs.
3. **Reputation-Gated Hiring**: Agencies define minimum reputation thresholds (e.g. 60+ for Verified Crypto Voices, 80+ for Tier-1 Alpha) to auto-gate applicants and ensure high-signal campaign reach.
4. **1-Click Free Cross-Verification**: Direct integration with **Sorsa** (`app.sorsa.io`) and **TwitterScore** (`twitterscore.io`) enables advertisers to perform instant due diligence without leaving the dApp.
5. **Secure Escrow Funding**: Advertisers commit the campaign budget into the intelligent contract escrow upfront.
6. **On-Chain Web Scrapers & LLM Consensus**: Validator nodes scrape live tweet URLs and reach consensus verifying text match, keywords, and hashtags against the pre-approved draft.
7. **Automated Milestone Payouts**: Completed phases automatically credit the creator's reentrancy-safe virtual ledger for instant withdrawal.
...
*   `test_x_account_linking_and_reputation`: Verifies the cryptographic challenge-tweet verification flow, on-chain AI reputation audits, and successful gated campaign application.
*   `test_reputation_gating_rejection`: Verifies that creators below the advertiser's minimum reputation threshold are reverted on-chain.
*   `test_anti_spoofing_prevent_stealing_handles`: Verifies anti-impersonation safeguards, blocking attackers from claiming or applying under another creator's verified X handle.
...
All tests run locally in under **a few seconds** and require absolutely no private credentials or setup.
